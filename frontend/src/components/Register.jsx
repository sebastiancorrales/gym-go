...
export default function Register({ onRegisterSuccess, onBackToLogin }) {
  ...
  // ── Shared styles ──────────────────────────────────────────────────────────
  const inputCls = 'w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition placeholder-gray-400';
            ...
            {[
              ...
              <div key={s.n} className={`flex items-start gap-3 transition-opacity ${step === s.n ? 'opacity-100' : 'opacity-40'}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5
                  ${step >= s.n ? 'bg-blue-500 text-white' : 'bg-white/10 text-gray-400'}`}>
                  {step > s.n ? (
